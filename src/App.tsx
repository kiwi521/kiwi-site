...
function App() {
  ...
  const storyCards = useMemo(
    () => [
      {
        eyebrow: 'Who I Am',
        title: 'A quiet observer building a personal archive through photographs.',
        body: '我把摄影当作长期练习：记录光线如何落下、颜色如何彼此衬托、以及日常里那些稍纵即逝却值得被留下的时刻。',
      },
      {
        eyebrow: 'What I Share',
        title: 'Notes on composition, atmosphere, light, and visual memory.',
        body: '这里会持续更新我在摄影学习中的判断、拆解、练习和审美笔记，让每一次拍摄都慢慢长成一套更清晰的观看方式。',
      },
      {
        eyebrow: 'Where To Follow',
        title: 'Across social platforms, the archive keeps expanding.',
    ...
    <div className="min-h-screen bg-white tracking-[-0.02em] text-white" style={{ fontFamily: "'Inter', sans-serif" }}>
        ...
        <div className="hidden md:flex absolute left-1/2 -translate-x-1/2 bg-white/20 backdrop-blur-md border border-white/30 rounded-full px-2 py-2 items-center gap-1">
          {navItems.map((item, index) => (
            <button
              key={item}
              className={index === 0
                ? 'px-4 py-1.5 rounded-full text-sm font-medium bg-white text-gray-950'
      ...
      <section className="relative w-full overflow-hidden h-screen bg-black" style={{ height: '100dvh' }}>
        ...
        <div className="absolute top-[14%] left-0 right-0 z-50 flex flex-col items-center text-center px-5 pointer-events-none">
          <p className="hero-anim hero-fade mb-5 inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-4 py-2 text-[11px] font-medium uppercase tracking-[0.26em] text-white/75 backdrop-blur-md"
             style={{ animationDelay: '0.12s' }}>
            kiwi · photography archive · visual notes
          </p>
        ...
        >
          <p className="text-sm text-white/80 leading-relaxed">
            Every frame I keep becomes a chapter of learning — from shifting light and still corners to the quiet details that teach me how to see more carefully.
          </p>
        </div>
        ...
        >
          <p className="text-xs sm:text-sm text-white/80 leading-relaxed">
            This homepage gathers my photography notes, visual references, and the platforms where I keep sharing new work, so the archive keeps growing beyond a single page.
          </p>
      ...
      <main className="relative bg-[#050505] px-5 pb-16 pt-14 sm:px-8 lg:px-12">
        <section className="mx-auto grid max-w-6xl gap-6 lg:grid-cols-[1.1fr_0.9fr]" id="journal">
          <div className="rounded-[2rem] border border-white/10 bg-white/[0.05] p-7 shadow-[0_30px_120px_rgba(0,0,0,0.35)] backdrop-blur-xl sm:p-10">
            <p className="mb-4 text-xs uppercase tracking-[0.26em] text-white/45">Current Focus</p>
          ...
          <div className="rounded-[2rem] border border-[#e8702a]/20 bg-[#120c08] p-7 shadow-[0_30px_100px_rgba(232,112,42,0.18)] sm:p-10">
            <p className="mb-4 text-xs uppercase tracking-[0.26em] text-[#f0b38d]">Elsewhere</p>
        ...
        <section className="mx-auto mt-6 grid max-w-6xl gap-6 md:grid-cols-3">
          {storyCards.map((card, index) => (
            ...
            >
              <p className="mb-4 text-xs uppercase tracking-[0.26em] text-white/45">{card.eyebrow}</p>
        ...
        <section className="mx-auto mt-20 flex max-w-6xl flex-col gap-8 rounded-[2.25rem] border border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.07),rgba(255,255,255,0.03))] px-7 py-10 shadow-[0_30px_120px_rgba(0,0,0,0.28)] sm:px-10 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-2xl">
            <p className="text-xs uppercase tracking-[0.26em] text-white/45">Quiet archive</p>
            <h2 className="mt-4 text-3xl font-semibold tracking-[-0.05em] text-white sm:text-4xl">
              A personal homepage shaped like a living field notebook.
            </h2>
